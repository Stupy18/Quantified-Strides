...
- **Mobile app** (`mobile/mobile/`) — scaffold, theme, component library, and navigation shell built; Load tab fully implemented with real API data (ATL/CTL/TSB SVG chart, ramp rate, workout history list); `BodyFreshnessMap` complete with camera-zoom, per-muscle freshness coloring, and anatomical detail shapes; History tab implemented (see below); Today, Log, Me tabs are placeholder stubs; bundle compiles but **renders a white screen** in Expo Go (runtime issue, unresolved). See `mobile/CLAUDE.md` → "Known Issue" section for diagnosis notes and debug steps.
- **Mobile History tab** — workout list wired to real API via `useWorkoutHistoryInfinite` (TanStack `useInfiniteQuery`); 5-item preview with "See all N →" / "Show less" toggle; cursor-based infinite pagination ("Load older →" / "↑ Show less" action row); filter chips (All / Run / Strength / MTB / Climb); Trends section is placeholder. Backend `GET /api/v1/training/workouts` extended with optional `before_date` cursor param and no upper day cap — existing callers unaffected.
